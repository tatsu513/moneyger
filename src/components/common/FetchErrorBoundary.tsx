...
class FetchErrorBoundary extends Component<Props, State> {
  ...
  render() {
    if (this.state.hasError) {
      return this.props.fallback ?? <FetchErrorFallback inline={this.props.inline} />
    }
...
const FetchErrorFallback: React.FC<{ inline: boolean }> = ({ inline }) => {
  if (inline) {
    return (
      <Typography variant="body2" align="left" display="inline" component="span" color={grey[700]}>
        読み込みに失敗しました
      </Typography>
    )
  }
